...
class LandCoverWorker(object):
    ...
    def pull_data(self, urls, trace_id, span_id):
        """
        ...
        with tracer.start_as_current_span("pull_data", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
    ...
    # TODO: Trace this function
    def redistribute(self, compression=False, trace_id=None, span_id=None):
        # Set directory of local TIFF files
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_as_current_span("redistribute", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
        ...
        # Import them lazily as a dask array
            ...
            # TODO: Trace this function
            def index_time_from_filenames(filenames, trace_id, span_id):
                function_context = trace.get_current_span().get_span_context()
                tracer = trace.get_tracer(__name__)
                context = self.get_parent_context(trace_id, span_id)
                with tracer.start_as_current_span("index_time_from_filenames", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
                    context = trace.get_current_span().get_span_context()
                    trace_id = context.trace_id
                    span_id = context.span_id
            ...
            # TODO: Trace this function
            def send_data_to_worker(data, times, top_left, worker,top_left_lat,top_left_long, trace_id, span_id):
                # print("Size of data before compression: ",data.nbytes)
                function_context = trace.get_current_span().get_span_context()
                tracer = trace.get_tracer(__name__)
                context = self.get_parent_context(trace_id, span_id)
                with tracer.start_as_current_span("send_data_to_worker", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
                    context = trace.get_current_span().get_span_context()
                    trace_id = context.trace_id
                    span_id = context.span_id
                    if compression:
                        ...
                        return worker.store_partition.remote(compressed_data, times, top_left, top_left_lat, top_left_long ,True, trace_id = trace_id, span_id = span_id)
                    else:
                        return worker.store_partition.remote(data, times, top_left,top_left_lat, top_left_long, False, trace_id = trace_id, span_id = span_id)

            # TODO: Trace this function
            def chunk(chunks={'x': 5000, 'y': 5000}, trace_id=None, span_id=None):
                function_context = trace.get_current_span().get_span_context()
                tracer = trace.get_tracer(__name__)
                context = self.get_parent_context(trace_id, span_id)
                # TODO: Move this somewhere better
                with tracer.start_as_current_span("chunk", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
                    context = trace.get_current_span().get_span_context()
                    trace_id = context.trace_id
                    span_id = context.span_id
                    xmin = 247530
                    ...
                    times = index_time_from_filenames(filenames, trace_id  = trace_id, span_id = span_id)  # This will hold the times to make the time dim
                    data_list = []
                    if len(filenames) == 0:
                    ...
                    node_partition = self.partition_nodes(x, trace_id = trace_id, span_id = span_id)
                    worker_partition = self.partition_workers(y, trace_id = trace_id, span_id = span_id)
                    self.global_timers["{}-{}-partitionData".format(self.node_id,self.id)].append(clock.time())
                    return node_partition, worker_partition, all_data

            # TODO: Trace this function
            def reshuffle(node_partition, worker_partition, da, trace_id, span_id):
                function_context = trace.get_current_span().get_span_context()
                tracer = trace.get_tracer(__name__)
                context = self.get_parent_context(trace_id, span_id)
                with tracer.start_as_current_span("reshuffle", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
                    context = trace.get_current_span().get_span_context()
                    trace_id = context.trace_id
                    span_id = context.span_id
                    res = []
                    ...
                    for hostname in self.assignments:
                        ...
                        for i in range(len(self.assignments[hostname])):
                            ...
                            res.append(send_data_to_worker(data, times, top_left, self.assignments[hostname][i], lat_coord, long_coord, trace_id = trace_id, span_id = span_id))
                            self.global_timers["{}-{}-sendDataToWorkers".format(self.node_id,self.id)].append(clock.time())
                    self.global_timers["{}-{}-reshuffle".format(self.node_id,self.id)].append(clock.time())
                    return res
            try:
                if len(filenames) > 0:
                    node_partition, worker_partition, da = chunk(trace_id = trace_id, span_id = span_id)
                    rslt = reshuffle(node_partition, worker_partition, da, trace_id=trace_id, span_id=span_id)
                    return rslt
                else:
                    return []
            except Exception as e:
    ...
    # TODO: Trace this function
    def apply_model(self, model: Model, trace_id, span_id) -> bool:
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_as_current_span("apply_model", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
            print("Hostname: {} top left: {}".format(self.hostname, self.top_left))
            return model.apply_model(self.data, self.times, self.top_left, trace_id, span_id)
    ...
    # TODO: Trace this function
    def store_partition(self, partition, times, top_left, top_left_lat, top_left_long, compression=False, trace_id = None, span_id = None):
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_as_current_span("store_partition", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
            if compression:
    ...
    # TODO: Trace this function
    def store_partition_idx(self, partition_idx, trace_id = None, span_id = None):
        ...
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_as_current_span("store_partition_idx", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
    ...
    # TODO: Trace this function
    def partition_nodes(self, x, trace_id, span_id):
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_as_current_span("partition_nodes", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
    ...
    # TODO: Trace this function
    def partition_workers(self, y, trace_id, span_id):
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_as_current_span("partition_workers", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
    ...
    # TODO: Trace this function
    def read_stored_data(self, trace_id, span_id):
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_as_current_span("read_stored_data", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
            os.chdir('/home/ray/raw/')
    ...
    # TODO: Trace this function
    def get_output_files(self,convertor:Convertor = None, trace_id = None, span_id = None):
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_as_current_span("get_output_files", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
    ...
    # TODO: Trace this function
    def populate_data(self, file_name, time_file_name, trace_id = None, span_id = None):
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_as_current_span("populate_data", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
    ...
    # TODO: Trace this function
    def delete_raw_folder(self, trace_id = None, span_id = None):
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_as_current_span("delete_raw_folder", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
            try:
    ...
    # TODO: Trace this function
    def reset_worker(self, trace_id = None, span_id = None):
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_as_current_span("reset_worker", context=context, kind=SpanKind.SERVER, links=[Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
            self.data = None
