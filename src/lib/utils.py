...
class Data(object):
    ...
    def apply_model(self, model: Model, trace_id, span_id):
        if len(self.model_results) == 0:  # Apply the model if not already
            for w in self.workers:
                self.model_results.append(w.apply_model.remote(model, trace_id, span_id))
...
class GoogleArchive(Archive):
    ...
    def get_urls_for_tile(self, data, trace_id, span_id):
        """
        ...
        with tracer.start_as_current_span("get_urls_for_tile", context=context, kind=trace.SpanKind.SERVER, links = [Link(function_context)] ):
    ...
    # TODO: Trace this function
    def fetch_urls(self, limit=None, parallel=False, per_tile=False, trace_id = None, span_id = None):
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_span('fetch_urls', context = context, kind=trace.SpanKind.SERVER, links = [Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
            self.unique_urls = []
            if parallel:
                ...
                for path in range(self.bound_box.get_start_path(), self.bound_box.get_end_path()-1, -1):
                    for row in range(self.bound_box.get_start_row(), self.bound_box.get_end_row()+1):
                        print("Getting urls for Path: {} Row: {}".format(path, row))
                        self.unique_urls.extend(self.get_urls_for_tile([path, row], trace_id = trace_id, span_id = span_id))
            if limit:
                self.unique_urls = self.unique_urls[:limit]
            return self

    # TODO: Trace this function
    def redistribute(self, compression=False, trace_id = None, span_id = None) -> Data:
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_span('redistribute', context = context, kind=trace.SpanKind.SERVER, links = [Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
            ...
            for worker in self.cluster_config.get_workers():
                res.append(worker.redistribute.remote(compression, trace_id, span_id))
            futures = ray.get(res)
            end_time = clock.time()
    ...
    # TODO: Trace this function
    def download(self, trace_id, span_id) -> Data:
        function_context = trace.get_current_span().get_span_context()
        tracer = trace.get_tracer(__name__)
        context = self.get_parent_context(trace_id, span_id)
        with tracer.start_span('download', context = context, kind=trace.SpanKind.SERVER, links = [Link(function_context)]):
            context = trace.get_current_span().get_span_context()
            trace_id = context.trace_id
            span_id = context.span_id
            ...
            for i in range(self.cluster_config.get_num_workers()):
                ...
                res.append(self.cluster_config.get_workers()[i].pull_data.remote(
                    self.unique_urls[start:start + true_url_per_worker[i]], trace_id, span_id))
                start += true_url_per_worker[i]
            ray.get(res)
            end_time = clock.time()
